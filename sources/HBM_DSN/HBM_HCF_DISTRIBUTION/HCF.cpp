...
int main(int argc, char**argv) {
	...
	* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
	...
	unsigned int iteration_number = (argc > 7)?(atoi(argv[7])):0;
		...
		for (unsigned int ch = 8; ch < (numChannels + 8); ch++) {
			for (unsigned int bank = 0; bank < numBanks; bank++) {
				for (unsigned int dp = 0; dp < numPatterns; dp++) {
					...
					snprintf(buf1, 80, "hcfirst_CH_%d_PC_%d_bank_%d_pattern_%s_%d.txt", ch, rankIdx, bank, pattern_names[dp].c_str(), iteration_number);
