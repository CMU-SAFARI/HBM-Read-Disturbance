...
void collect_bitflips(vector<uint>& bitflips, const char* read_data, const bitset<256>& input_data_pattern) {

    bitflips.clear();

    bitset<256> read_data_bitset;

    uint32_t* iread_data = (uint32_t*) read_data;

    // check for bitflips in each cache line
    for(int cl = 0; cl < ROW_SIZE/32; cl++) {

        read_data_bitset.reset();
        for(int i = 0; i < 256/32; i++) {
            bitset<256> tmp_bitset = iread_data[cl*(256/32*2) + i];

            read_data_bitset |= (tmp_bitset << i*32);
        }

        // compare and print errors
        bitset<256> error_mask = read_data_bitset ^ input_data_pattern;
...
bitset<256> setup_data_pattern(const uint data_pattern_type) {
    
    bitset<256> data_pattern;
...
void init_data_row_range(Program& prog, SoftMCRegAllocator& reg_alloc, const SMC_REG reg_bank_addr, const SMC_REG reg_num_cols, 
    const uint first_row_id, const uint last_row_id, const bitset<256>& data_patt) {

    uint initial_free_regs = reg_alloc.num_free_regs();

    bitset<256> bitset_int_mask(0xFFFFFFFF);

    prog.add_inst(SMC_LI(1, CASR)); // Load 8 into CASR since each WRITE writes 8 columns
...
void init_row_data(Program& prog, SoftMCRegAllocator& reg_alloc, const SMC_REG reg_bank_addr, const SMC_REG reg_num_cols, 
    const vector<LogicalRowID>& rows_to_init, const vector<bitset<256>>& data_patts) {
...
void hammer_aggressors(Program& prog, SoftMCRegAllocator& reg_alloc, const SMC_REG reg_bank_addr, const vector<uint>& aggressors,
                        const std::vector<uint>& hammers_per_aggressor, const uint aggr_bank_id, std::vector<LogicalRowID> dummy_rows, const uint hammers_per_dummy, 
                        const bool hammer_dummies_independently, const bool hammer_dummies_before, const bool hammer_dummies_after, const bool shift_dummies_after, const std::vector<uint> dummy_banks,
    ...
    if (trrref_sync) { 
        ...
        if(hammer_dummies_independently && hammer_dummies_after){
            std::vector<uint> dummy_hammers;
            for (uint i = 0; i < dummy_rows.size(); i++){
                // add dummy hammer counts, one hammer count per dummy
                dummy_hammers.push_back(hammers_per_dummy);
                dummy_rows[i] = shift_dummies_after ? (dummy_rows[i] + 5) : dummy_rows[i]; // increment dummy row IDs
            }
            perform_hammers(prog, reg_alloc, reg_bank_addr, dummy_rows, dummy_hammers, 0, aggr_bank_id, cascaded_hammer_dummy, dummy_banks, fake_hammer, fake_dummy_hammer);
        }
...
void read_row_data(Program& prog, SoftMCRegAllocator& reg_alloc, const SMC_REG reg_bank_addr, const SMC_REG reg_num_cols, 
    ...
    prog.add_inst(SMC_LI(1, CASR)); // Load 8 into CASR since each READ reads 8 columns
    SMC_REG reg_row_addr = reg_alloc.allocate_SMC_REG();
...
void performHammer(SoftMCPlatform& platform, const uint target_bank, const PhysicalRowID anchor_row, const std::string row_layout, 
                    ...
                    const bool hammer_dummies_independently, const bool hammer_dummies_before, const bool hammer_dummies_after, const bool shift_dummies_after, const std::vector<uint> dummy_banks,
                    const uint num_refs, const uint refs_per_loop, const bool trrref_sync, const bool cascaded_hammer_aggr, const bool cascaded_hammer_dummy, 
                    const bitset<256>& victim_data, const bitset<256>& aggr_data, 
    ...
    prog.add_inst(SMC_LI(NUM_COLS_PER_ROW, reg_num_cols));
    ...
    std::vector<bitset<256>> data_patts;
    ...
    hammer_aggressors(prog, reg_alloc, reg_bank_addr, aggr_ids, num_hammers, target_bank, dummy_rows, hammers_per_dummy, 
        hammer_dummies_independently, hammer_dummies_before, hammer_dummies_after, shift_dummies_after, dummy_banks, num_refs, refs_per_loop, 
...
bool testHammerCount(SoftMCPlatform& platform, const uint bank_id, const uint row_id, const uint hammer_count,
    ...
    std::vector<bitset<256>> aggr_data_pattern = std::vector<bitset<256>>(aggr_ids.size(), aggr_data);
...
uint findHCFirst(SoftMCPlatform& platform, const uint bank_id, const uint row_id, const bitset<256>& victim_data, const bitset<256>& aggr_data){

    const uint INIT_HAMMER_COUNT = 5000;
    const uint MAX_HAMMER_COUNT = 2000000;
...
uint syncTRRREF(SoftMCPlatform& platform, const uint input_data_victims, const uint input_data_aggressors) {
    ...
    bitset<256> victims_data = setup_data_pattern(input_data_victims);
    bitset<256> aggrs_data = setup_data_pattern(input_data_aggressors);
    uint TARGET_BANK = 1;
    uint TARGET_ROW;

    while (hc_first == 0) {
        TARGET_ROW = rand() % NUM_ROWS;
        std::cout << BLUE_TXT << "Finding HC_first for bank: " << TARGET_BANK << ", row: " << TARGET_ROW << "..." << NORMAL_TXT << std::endl;
        hc_first = findHCFirst(platform, TARGET_BANK, TARGET_ROW, victims_data, aggrs_data);
    ...
    std::cout << YELLOW_TXT << "The last run without bitflips: " << last_no_bitflip_run << NORMAL_TXT << std::endl;

    uint num_last_regular_refs = NUM_RUNS - last_no_bitflip_run - 1;
    uint refs_to_sync = (most_frequent_distance - num_last_regular_refs) % 17;
...
void hammerBank(SoftMCPlatform& platform, const uint target_bank, const PhysicalRowID first_row_id, const PhysicalRowID last_row_id, 
        ...
        const uint num_dummies, const bool hammer_dummies_independently, const bool hammer_dummies_before, const bool hammer_dummies_after, const bool shift_dummies_after, const std::vector<uint>& dummy_banks,
    ...
    const uint TREFI = DEFAULT_TREFI; // assume tREFI = 7800ns
    const uint TRC = 48; // assume activate-precharge complete in 50ns
    ...
    // // if(hammer_dummies_before && hammer_dummies_after)
    // //     hammers_per_dummy /= 2;

    uint total_dummy_hammers = 0;
    uint total_aggr_hammers = 0;
    if(hammer_dummies_before){
        std::cout << BLUE_TXT << "Number of before dummies: " << num_dummies << NORMAL_TXT << std::endl;
        std::cout << BLUE_TXT << "Hammers per before dummy (per bank): " << hammers_per_dummy << NORMAL_TXT << std::endl;
        total_dummy_hammers += hammers_per_dummy*num_dummies*dummy_banks.size();
    }
    std::cout << BLUE_TXT << "Number of aggressors: " << num_hammers.size() << NORMAL_TXT << std::endl;
    std::cout << BLUE_TXT << "Aggressor hammers: " << NORMAL_TXT;
    for(uint i = 0; i < num_hammers.size(); i++){
        std::cout << BLUE_TXT << num_hammers[i] << " " << NORMAL_TXT;
        total_aggr_hammers += num_hammers[i];
    }
    std::cout << std::endl;
    if(hammer_dummies_after){
        std::cout << BLUE_TXT << "Number of after dummies: " << num_dummies << NORMAL_TXT << std::endl;
        std::cout << BLUE_TXT << "Hammers per after dummy (per bank): " << hammers_per_dummy << NORMAL_TXT << std::endl;
        total_dummy_hammers += hammers_per_dummy*num_dummies*dummy_banks.size();
    }

    std::cout << BLUE_TXT << "Total dummy hammers: " << total_dummy_hammers << NORMAL_TXT << std::endl;
    std::cout << BLUE_TXT << "Total aggressor hammers: " << total_aggr_hammers << NORMAL_TXT << std::endl;
    std::cout << BLUE_TXT << "Total hammers: " << total_dummy_hammers + total_aggr_hammers << NORMAL_TXT << std::endl;
    ...
    for(PhysicalRowID anchor_row = first_row_id; anchor_row <= last_row_id; anchor_row++){
        ...
        performHammer(platform, target_bank, anchor_row, row_layout, num_hammers, dummy_rows, hammers_per_dummy, hammer_dummies_independently, hammer_dummies_before, hammer_dummies_after, shift_dummies_after, dummy_banks,
        ...
        platform.receiveData(buf, ROW_SIZE*victims_pos.size()*2);
        ...
        for (uint i_victim = 0; i_victim < victims_pos.size(); i_victim++) {
            // check for bitflips
            collect_bitflips(bitflips, buf + i_victim*ROW_SIZE*2, victims_data);
    ...
    total_dummy_hammers = 0;
    total_aggr_hammers = 0;
    if(hammer_dummies_before){
        std::cout << BLUE_TXT << "Number of before dummies: " << num_dummies << NORMAL_TXT << std::endl;
        std::cout << BLUE_TXT << "Hammers per before dummy (per bank): " << hammers_per_dummy << NORMAL_TXT << std::endl;
        total_dummy_hammers += hammers_per_dummy*num_dummies*dummy_banks.size();
...
int main(int argc, char** argv)
{
    ...
    bool shift_dummies_after = false;
    ...
    uint input_data_victims = 1;
    uint input_data_aggressors = 2;
    ...
    desc.add_options()
        ...
        ("shift_refs", value(&shift_refs)->default_value(shift_refs), "When specified, a single REF is issued at the beginning of the experiment.")
        ...
        ("shift_dummies_after", bool_switch(&shift_dummies_after)->default_value(shift_dummies_after), "When specified, the the dummy rows are hammered after the actual aggressor rows. Can be specified together with --hammer_dummies_before to hammer the dummies both before and after.")
    ...
    if(shift_refs > 0)
        issue_REFs(platform, shift_refs);

    hammerBank(platform, target_bank, row_range[0], row_range[1], num_hammers, num_ref_loops, refs_per_loop, trrref_sync, 
        num_dummy_rows, hammer_dummies_independently, hammer_dummies_before, hammer_dummies_after, shift_dummies_after, dummy_banks,
