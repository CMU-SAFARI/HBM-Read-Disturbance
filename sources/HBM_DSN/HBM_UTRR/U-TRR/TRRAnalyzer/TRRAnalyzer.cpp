...
float DEFAULT_TRFC = 260.0f * 2; // ns
...
void collect_bitflips(vector<uint>& bitflips, const char* read_data, const bitset<256>& input_data_pattern, const vector<uint> bitflips_loc) {
    ...
    if(bitflips_loc.size() == 0){
        ...
        for(auto bitflip: bitflips_loc){
            ...
            // std::cout << "bitflip: " << bitflip << std::endl;
            // std::cout << "read_data_bitset: " << read_data_bitset << std::endl;
            // std::cout << "input_data_pattern: " << input_data_pattern << std::endl;
            // std::cout << "error_mask: " << error_mask << std::endl;
...
void writeToDRAM(Program& program, const uint target_bank, const uint start_row, 
    ...
    program.add_inst(SMC_LI(NUM_COLS_PER_ROW, REG_NUM_COLS));

    program.add_inst(SMC_LI(1, CASR)); // Load 8 into CASR since each READ reads 8 columns
...
void parse_all_weaks(boost::filesystem::ifstream& f_row_groups, vector<WeakRowSet>& row_groups) {
    ...
    // std::cout << "Parsed " << row_groups.size() << " weak row sets" << std::endl;
}
...
void init_row_data(Program& prog, SoftMCRegAllocator& reg_alloc, const SMC_REG reg_bank_addr, const SMC_REG reg_num_cols, 
    const vector<uint>& rows_to_init, const vector<bitset<256>>& data_patts) {
...
void init_row_data(Program& prog, SoftMCRegAllocator& reg_alloc, const SMC_REG reg_bank_addr, const SMC_REG reg_num_cols, 
    const uint target_row, const bitset<256>& data_pattern) {

    vector<uint> rows_to_init{target_row};
    vector<bitset<256>> data_patts{data_pattern};
...
void init_hammerable_row_set(Program& prog, SoftMCRegAllocator& reg_alloc, const SMC_REG reg_bank_addr, const SMC_REG reg_num_cols, 
    ...
    bitset<256> aggr_data_patt = hr.data_pattern;
    aggr_data_patt.flip();
...
bool is_hammerable(SoftMCPlatform& platform, const WeakRowSet& wrs, const std::string row_layout, const bool cascaded_hammer) {
    ...
    SMC_REG reg_num_cols = reg_alloc.allocate_SMC_REG();
    p_testRH.add_inst(SMC_LI(NUM_COLS_PER_ROW, reg_num_cols));

    p_testRH.add_inst(SMC_LI(1, CASR)); // Load 8 into CASR since each READ reads 8 columns
    ...
    /*********************************************/
    char buf[ROW_SIZE*hr.victim_ids.size()*2];
    platform.receiveData(buf, ROW_SIZE*hr.victim_ids.size()*2);
    ...
    for(uint i = 0; i < hr.victim_ids.size(); i++) {
        bitflips.clear();
        collect_bitflips(bitflips, buf + ROW_SIZE*i*2, hr.data_pattern, vector<uint> {});

        if(bitflips.size() == 0 && ((hr.victim_ids[i] - hr.aggr_ids[0]) == 1 || (hr.victim_ids[i] - hr.aggr_ids[0]) == -1)) {
...
vector<vector<uint>> analyzeTRR(SoftMCPlatform& platform, const vector<HammerableRowSet>& hammerable_rows, const vector<uint>& dummy_aggrs, 
    ...
    prog_read->add_inst(SMC_LI(NUM_COLS_PER_ROW, reg_num_cols));
    ...
    if(!use_single_softmc_prog) {
        ...
        for (auto& hrs : hammerable_rows) {
            for(uint vict_ind = 0; vict_ind < hrs.victim_ids.size(); vict_ind++) {
                ...
                bitflips.clear();
                collect_bitflips(bitflips, buf, hrs.data_pattern, hrs.vict_bitflip_locs[vict_ind]);
                row_it++;
...
void pick_hammerable_row_groups_from_file(SoftMCPlatform& platform, boost::filesystem::ifstream& f_row_groups, vector<WeakRowSet>& row_groups, const uint num_row_groups,
    ...
    while(row_groups.size() != num_row_groups) {
        ...
        //     std::cout << "B: " << wr.bank_id << ", R: " << wr.rows_as_str() << ", ret: " << wr.ret_ms << " ms" << std::endl;
        // }
...
int main(int argc, char** argv)
{
    ...
    if(!use_single_softmc_prog) {
        ...
        ulong read_data_size = ROW_SIZE*total_victims*2;
        char* buf = new char[read_data_size];
        vector<uint> bitflips;
        ...
        for (uint i = 0; i < num_iterations; i++) {
            if(!skip_hammering_aggr) {
                ...
                for (auto& hr : hrs) {
                    for(uint vict_ind = 0; vict_ind < hr.victim_ids.size(); vict_ind++) {
                        bitflips.clear();
                        collect_bitflips(bitflips, buf + row_it*ROW_SIZE*2, hr.data_pattern, hr.vict_bitflip_locs[vict_ind]);
                        row_it++;
                    ...
                    for(uint uni_ind = 0; uni_ind < hr.uni_ids.size(); uni_ind++) {
                        bitflips.clear();
                        collect_bitflips(bitflips, buf + row_it*ROW_SIZE*2, aggr_data_pattern, hr.uni_bitflip_locs[uni_ind]);
                        row_it++;
