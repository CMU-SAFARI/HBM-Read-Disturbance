...
#define HC_FIRST_RESOLUTION 1

#define P_A_1  0x00000000
#define P_VICTIM  ~P_A_1
#define P_A_2 P_A_1

#define HAMMER_COUNT 16
#define MAX_HAMMER_COUNT 16
...
void double_rowhammer_test(unsigned int channel, unsigned int rankIdx, unsigned int startBank, unsigned int hammerCount, unsigned int patternAggressor1, unsigned int patternAggressor2, unsigned int patternVictim, unsigned int agr1, unsigned int agr2, unsigned int victim, unsigned int RAS_scale) {
	...
	// --------------------- Start Hammering ---------------------
		...
		p.add_label("HAMMER");
			...
			p.add_inst(SMC_NOP(), SMC_NOP(), SMC_ACT(BAR, 0, RAR, 0, rankIdx), SMC_NOP());
			p.add_inst(SMC_SLEEP(5));
			// branch will already wait some for us
			if (RAS_scale >= 2)
...
int main(int argc, char**argv) {
	...
	* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
	...
	unsigned int ras_scale_list[20] = {0, 1, 2, 4, 8, 16, 32, 64, 118, 256, 512, 1063, 5000, 10000, 20000, 50000, 100000, 200000, 500000, 1000000};
	unsigned int hammer_count_list[20] = {256000, 128000, 128000, 128000, 128000, 64000, 64000, 32000, 32000, 16000, 6400, 3200, 1600, 512, 256, 256, 32, 32, 16, 16};
					...
					for (unsigned int tAggOn = 0; tAggOn < 20; tAggOn++) {
						...
						for (unsigned int row = 0; row < numRows; ++row) {
							...
							for (unsigned int iterations = 0; iterations < NUM_ITERATIONS; ++iterations) {
								hammerCount = hammer_count_list[tAggOn];
								upper_hammerCount = hammer_count_list[tAggOn];
								while (true) {
									row_errors[row] = 0;
									...
									if (row_errors[row] >= 1) {
										...
										if (hammerCount == hammer_count_list[tAggOn])
											break;
						...
						snprintf(buf1, 100, "hcfirst_CH_%d_PC_%d_bank_%d_pattern_%d_tAggOn_%d.txt", ch, rankIdx, bank, dp, tAggOn);
